...
Talos가 Kubernetes 노드를 운영하는 방식과 그 배경에 있는 설계 철학을 이해합니다. ephemeral QEMU 환경을 `talosctl`로 관찰해 개념과 실제 상태를 연결합니다.

운영 절차를 외우기보다 Talos가 어떤 문제를 해결하고, 그 선택이 어떤 장점과 제약을 만드는지 설명하는 것이 목표입니다.

## 준비 방향

machine configuration, API 중심 관리, 노드 lifecycle을 하나의 운영 모델로 연결합니다. QEMU 실습은 이 모델을 확인하기 위한 작은 관찰 환경으로 사용하며, 구체적으로 무엇을 보여줄지는 준비팀이 선택합니다.
...
- Kubernetes 노드에 특화된 operating system이라는 선택은 무엇을 가능하게 하는가?
...
- 변경의 종류에 따라 적용 방식이나 lifecycle 전환이 달라지는 이유는 무엇인가?
...
공식 문서를 바탕으로 ephemeral Talos 환경을 준비하고, 앞에서 만든 모델 가운데 일부를 확인합니다. 예를 들어 머신 상태, 서비스, 로그, 실제 적용된 설정, 재부팅 전후의 변화 등을 선택할 수 있습니다.

호스트 operating system과 CPU 아키텍처에 따라 QEMU 실행 조건이 다를 수 있으므로, 시연 환경과 대체 방법을 미리 확인합니다. 운영 환경의 인증 정보나 설정은 사용하지 않습니다.
...
- 머신 설정과 원하는 상태
- API 접근과 신뢰 모델
- 노드 lifecycle과 관찰 가능한 상태
- ephemeral QEMU 환경에서의 `talosctl` 관찰

### 범위 밖

- SCG repository의 `k` 명령
