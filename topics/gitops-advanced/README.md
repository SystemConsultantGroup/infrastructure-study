...
선언적 reconciliation과 bootstrap, 변경, 복구 과정에서 필요한 명령형 작업의 관계를 중심으로 준비합니다. 저장소와 `k --help`를 일차 자료로 사용하되, 구조를 디렉터리 순서대로 소개하지는 않습니다.

이 주제를 준비하거나 진행하는 동안 운영 클러스터에 변경 또는 파괴 작업을 실행하지 않습니다. 시연이 필요하다면 읽기 전용, 로컬, 일회용 환경을 사용합니다.
...
- 생성된 산출물과 수정해야 할 원본 선언을 어떻게 구분할 수 있는가?
- 여러 컨트롤러와 외부 시스템 사이의 소유권은 어디에서 나뉘는가?
...
- 추적 중인 브랜치의 변경이 운영 상태에 미치는 영향은 무엇인가?
- Secret은 저장, 복호화, 실제 값으로의 변환, 실행 환경 전달 과정에서 누구의 신뢰를 필요로 하는가?

실제 Secret 값보다 식별자, 키, 컨트롤러, 외부 서비스 사이의 신뢰 관계에 집중합니다.
...
- 명령형 작업이 끝난 뒤 상태 관리 책임을 다시 선언적 reconciliation에 맡기려면 무엇이 필요한가?

분류 방식과 대표 명령은 준비팀이 선택합니다. 전체 명령 목록을 만들 필요는 없습니다.

### Bootstrap과 복구 경계

- Argo CD와 플랫폼 컨트롤러가 동작하기 전에 무엇이 준비되어야 하는가?
- 선언적 시스템을 시작하기 위해 불가피하게 명령형인 단계는 무엇인가?
- 클러스터를 다시 만들거나 복구하려면 Git 밖의 어떤 상태가 필요한가?
- Bootstrap이 끝났다는 것은 소유권 관점에서 무엇을 의미하는가?

### 장애 판단

준비팀은 병합 후 불수렴, Secret 제공 실패, 노드 교체, 부분 업그레이드, 수동 변경으로 인한 상태 불일치 등을 참고해 적절한 사례를 선택합니다.

- 기준이 되는 desired state는 무엇인가?
- 판단에 필요한 근거는 어디에서 얻는가?
- Reconciliation을 계속할지 멈출지 어떤 기준으로 결정하는가?
- 필요한 명령형 작업의 위험과 복구 완료를 어떻게 판단하는가?
...
- 저장소의 최상위 Application, 플랫폼 및 애플리케이션 선언, 검증 워크플로, 패치, `scripts/k` 관련 문서와 코드
- `k --help`
