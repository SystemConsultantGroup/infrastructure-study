...
참가자는 새로운 환경에서도 저장소가 제공하는 개발 셸을 사용할 수 있어야 합니다. 또한 저장소가 보장하는 상태와 호스트에 남아 있는 차이를 구분할 수 있어야 합니다.
...
- 이후 차시를 진행하려면 어떤 운영체제, 권한, 네트워크 접근이 필요한가?
- Linux를 직접 사용하는 경우와 가상 환경을 사용하는 경우에는 어떤 차이가 있는가?
- 참가자가 미리 알아야 할 셸과 파일 시스템 개념은 어느 정도인가?

### 명령형 환경과 선언형 환경

- 도구를 하나씩 수동 설치하는 방식은 무엇에 의존하는가?
- 명령형(imperative) 방식의 절차 가운데 무엇을 선언형(declarative) 상태로 옮길 수 있는가?
...
- 개발 셸에 들어가고 나올 때 호스트에는 무엇이 남는가?

### 실제 사용과 문제 해결

준비팀은 초기 환경에서 저장소의 개발 셸까지 도달하는 흐름을 보여줍니다. 구체적인 시연 방식과 확인할 도구는 자유롭게 정합니다.
